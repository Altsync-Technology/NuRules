...
## 基本
- 尽可能使用使用 lombok
- 对于新增的功能，使用 @Nullable 来处理可能为 `null` 的字段，用 Optional<T> 来包装可能为 null 的函数返回值。
## 语法与逻辑
- 局部变量类型尽量使用 `var` 
- 使用四格空格缩进
...
```
- 不要一开始就给每个服务新建接口，到出现多个实现的时候再提取函数到接口
- 几乎没有什么必要使用工厂模式和单例模式
- 处于历史原因，我们依旧使用 `@Autowired` 注入服务
- 尽量链式调用
...
public List<ShitInfo> grabShitInfos(List<Integer> shitId) 
{
    ...
    // 1次查询
    var shits = shitService.selectList(shitId);
